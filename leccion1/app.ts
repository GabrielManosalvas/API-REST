...
// =======================================
async function obtenerPostPorId(id: number): Promise<Post> {
...
// =======================================
async function obtenerPostsPorUsuario(userId: number): Promise<Post[]> {
...
// =======================================
async function crearPost(post: Post): Promise<Post> {
  const respuesta = await fetch(API_URL, {
    method: "POST",
    headers: {
...
async function ejecutarDemo(): Promise<void> {
  ...
  try {
    ...
    const postActualizado = await actualizarTitulo(
      postCreado.id ?? 1,
      "Título actualizado"
    );
